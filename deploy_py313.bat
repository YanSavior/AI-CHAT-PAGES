...
echo.
echo 升级setuptools和wheel...
pip install --upgrade setuptools wheel

echo.
echo 安装兼容Python 3.13的依赖包...
echo 使用 requirements_py313.txt 文件...
pip install -r requirements_py313.txt
if errorlevel 1 (
    echo.
    echo ⚠️  部分依赖安装失败，尝试使用预编译包...
    echo.
    echo 尝试安装预编译的wheel包...
    pip install --only-binary=all -r requirements_py313.txt
    if errorlevel 1 (
        echo.
        echo ❌ 依赖安装仍然失败
        echo.
        echo 建议解决方案：
        echo 1. 使用Python 3.11或3.12（更稳定）
        echo 2. 或者使用conda环境
        echo 3. 或者手动安装每个包
        echo.
        pause
